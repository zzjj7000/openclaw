...
export function buildAgentSystemPrompt(params: {
  ...
  const isWindows = params.runtimeInfo?.os?.toLowerCase().includes("windows") ?? false;
  const coreToolSummaries: Record<string, string> = {
    ...
    grep: isWindows
      ? "Search file contents (Windows PowerShell: use `Select-String` or its alias `sls`)"
      : "Search file contents for patterns",
    find: "Find files by glob pattern",
    ls: isWindows
      ? "List directory contents (Windows PowerShell: use `ls`, `dir`, or `Get-ChildItem`)"
      : "List directory contents",
    exec: isWindows
      ? "Run shell commands (Windows PowerShell; standard aliases `ls`, `cat`, `rm` available)"
